...
from models import GameplayMonthDeltaItem, GameplayItem
    ...
    if create_type == "gameplay_delta":
        ...
        for batch_start in tqdm(range(0, len(existing_gameplay_list), CREATE_BATCH_SIZE)):
            ...
            for current_gameplay in current_items:
                if previous_gameplay := previous_month_gameplay_list.get(current_gameplay.steamid):
                    previous_gameplay_item_dict = {
                       item.appid: item for item in previous_gameplay.gameplay_list
                    }
                    empty_gameplay_item = GameplayItem(appid="0",playtime=0)
                    gameplay_delta_item_list = [GameplayMonthDeltaItem(appid=gameplay_item.appid, playtime=(gameplay_item.playtime - previous_month_gameplay_dict.get(gameplay_item.appid, GameplayItem(appid=gameplay_item.appid, playtime=0)))) for gameplay_item in current_gameplay.gameplay_list]
            repo.
